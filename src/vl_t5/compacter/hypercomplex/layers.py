...
from .inits import glorot_uniform, glorot_normal, init_ones  # phm_init
...
class PHMLinear(torch.nn.Module):
    def __init__(self,
        ...
        self.shared_W_phm = shared_W_phm  # False
        ...
        if not self.shared_W_phm:
            if self.factorized_phm:
                ...
                self.W = nn.Parameter(torch.Tensor(size=(
                    phm_dim, self._in_feats_per_axis, self._out_feats_per_axis)),
                    requires_grad=True
                )  # [phm_dim, ]
    ...
    def set_phm_rule(self, phm_rule=None, phm_rule_left=None,
                     phm_rule_right=None):
        """If factorized_phm_rules is set, phm_rule is a tuple, showing the left
        and right phm rules, and if this is not set,
        this is showing the phm_rule."""
        if self.factorized_phm_rule:
