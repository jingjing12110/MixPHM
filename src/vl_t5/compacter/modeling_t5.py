...
class T5Attention(nn.Module):
    def __init__(self, config: T5Config, has_relative_attention_bias=False,
        ...
        self.gradient_checkpointing = getattr(config, "gradient_checkpointing",
                                              False)
...
class T5Block(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False,
        ...
        if self.is_decoder:
            self.layer.append(
                T5LayerCrossAttention(config, adapter_config=adapter_config))
...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None, adapter_config=None,
        ...
        self.block = nn.ModuleList(
            [T5Block(self.per_layer_config(config, i, self.adapter_config,
                                           self.is_decoder),
                     has_relative_attention_bias=bool(i == 0),
                     adapter_config=adapter_config) for i in
        ...
        if inputs_embeds is None:
            ...
            if self.append_prefix:
                inputs_embeds = torch.cat([self.prefix_emb.unsqueeze(0).repeat(
                    inputs_embeds.shape[0], 1, 1), inputs_embeds],
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config, adapter_config=None):
        ...
        ###########################################################################
        # Creates and sets a shared phm_rule in case of hypercomplex adapters
        # with a shared phm_rule.
        if config.train_task_adapters and adapter_config.hypercomplex_adapters:
            if adapter_config.shared_phm_rule:
                ...
                if self.factorized_phm_rule:
                    self.phm_rule_left = nn.Parameter(
                        torch.FloatTensor(phm_dim, phm_dim, 1).to(
                            adapter_config.device),
                        requires_grad=adapter_config.learn_phm)
                    self.phm_rule_right = nn.Parameter(
                        torch.FloatTensor(phm_dim, 1, phm_dim).to(
                            adapter_config.device),
                        requires_grad=adapter_config.learn_phm)
                    if adapter_config.phm_c_init == "normal":
                        self.phm_rule_left.data.normal_(mean=0,
                                                        std=adapter_config.phm_init_range)
                        self.phm_rule_right.data.normal_(mean=0,
                                                         std=adapter_config.phm_init_range)
                    ...
                    self.phm_rule = nn.Parameter(
                        torch.FloatTensor(phm_dim, phm_dim, phm_dim).to(
                            adapter_config.device),
                        requires_grad=adapter_config.learn_phm)
                    if adapter_config.phm_c_init == "normal":
                        self.phm_rule.data.normal_(mean=0,
                                                   std=adapter_config.phm_init_range)
                    elif adapter_config.phm_c_init == "uniform":
            ...
            if adapter_config.shared_W_phm:
                ...
                if self.factorized_phm:
                    ...
                    self.W_down_left = nn.Parameter(torch.Tensor(
                        size=(self.phm_dim, in_feats_per_axis, self.phm_rank)),
                        requires_grad=True)
                    self.W_down_right = nn.Parameter(torch.Tensor(
                        size=(self.phm_dim, self.phm_rank, out_feats_per_axis)),
                        requires_grad=True)
                    self.W_up_left = nn.Parameter(torch.Tensor(
                        size=(self.phm_dim, out_feats_per_axis, self.phm_rank)),
                        requires_grad=True)
                    self.W_up_right = nn.Parameter(torch.Tensor(
                        size=(self.phm_dim, self.phm_rank, in_feats_per_axis)),
                        requires_grad=True)
                    self.init_W(in_feats_per_axis, out_feats_per_axis,
                                W_left=self.W_down_left,
                                W_right=self.W_down_right)
                    self.init_W(out_feats_per_axis, in_feats_per_axis,
                                W_left=self.W_up_left,
                                W_right=self.W_up_right)
                else:
                    self.W_down = nn.Parameter(torch.Tensor(size=(
                        self.phm_dim, in_feats_per_axis, out_feats_per_axis)),
                        requires_grad=True)
                    self.W_up = nn.Parameter(torch.Tensor(size=(
                        self.phm_dim, out_feats_per_axis, in_feats_per_axis)),
                        requires_grad=True)
                    self.init_W(in_feats_per_axis, out_feats_per_axis,
                                W=self.W_down)
                    self.init_W(out_feats_per_axis, in_feats_per_axis,
    ...
    def init_prefix_weights(self):
        if self.init_prefix_from_vocab:
            self.prefix_shared.data = self.get_input_embeddings().weight[
                                      :self.prefix_dim].clone().detach()
        else:
            random_range = 0.5
    ...
    def init_W(self, in_feats_per_axis, out_feats_per_axis, W=None, W_left=None,
               W_right=None):
        ...
        if self.prefix_tuning:
            if attention_mask is not None:
                attention_mask = torch.cat([torch.ones(
                    (attention_mask.shape[0], self.prefix_dim)).to(
