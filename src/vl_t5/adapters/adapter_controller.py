...
class AdapterController(nn.Module):
    ...
    def construct_adapters(self, tasks):
        ...
        if self.use_single_adapter:
            ...
            if self.share_up_sampler:
                ...
                # extract the layer of the adapters for first task
                for task in tasks:
            ...
            if self.hypercomplex_adapters and self.shared_phm_rule_over_tasks \
    ...
    def forward(self, inputs, task):
        ...
        adapter = self.get_adapter(task)
        z = self.pre_layer_norm(inputs) if self.add_layer_norm_before_adapter \
            else inputs
        outputs = adapter(z)
        if self.add_layer_norm_after_adapter:
