...
from .compacter.hypercomplex.layers import PHMLinear
from .compacter.hypercomplex.inits import glorot_uniform, glorot_normal

from losses.r2_loss import off_diagonal
from redundancy.mi import compute_skl_masked_loss
...
class VisualEmbedding(nn.Module):
    def __init__(self, config, obj_order_embedding):
        ...
        if self.config.individual_vis_layer_norm:
            ...
            absolute_vis_pos_embedding = [nn.Linear(
                pos_dim + 1, config.d_model,
                bias=False if not self.bitfit else True
            )]
            if self.config.use_vis_layer_norm:
    ...
    def forward(self, feats, pos, img_order_ids=None, obj_order_ids=None):
        ...
        if self.config.use_vis_order_embedding:
            ...
            obj_order_ids = \
                self.obj_order_embedding.num_embeddings - obj_order_ids - 1
...
class JointEncoder(T5Stack):
    def __init__(self, config, embed_tokens=None):
        ...
        if self.config.use_adapter_r2loss:
            from losses.r2_loss import R2Loss
        ...
        if self.training and self.config.use_adapter_r2loss:
            for n_l in range(len(ups)):
                ...
                if self.config.using_mi_loss:
                    ...
                    # loss_mi = compute_skl_masked_loss(
                    #     ups[n_l], hidden_states, attention_mask.int())
...
class VLT5(T5ForConditionalGeneration):
    ...
    def __init__(self, config):
        ...
        # ******************************************************************
        if config.apply_compacter and config.hypercomplex_adapter:
                ...
                if self.factorized_phm_rule:
                    ...
                    if config.phm_c_init == "normal":
                        self.phm_rule_left.data.normal_(
                            mean=0, std=config.phm_init_range)
    ...
    def extend_vocab(self, vocab_size):
        ...
        new_lm_head = nn.Linear(
            self.config.d_model, vocab_size,
            bias=False if not self.bitfit else True
        )
    ...
    def forward(
        ...
        decoder_outputs = self.decoder(
            ...
            encoder_hidden_states=hidden_states,  # [bs, seq_len, 768]
        ...
        ada_r2loss = None
        adapter_align_loss = None
        adapter_red_loss = None
        if self.training and self.config.use_adapter_r2loss:
            adapter_red_loss = self.config.red_w * 0.5 * (
                    torch.stack(encoder_outputs.ada_red_loss).mean() +
                    torch.stack(decoder_outputs.ada_red_loss).mean()
            )

            # # MI loss
            # from redundancy.mi import MILoss
            # loss_func = MILoss()
            #
            # # attn_vis_mask = torch.ones((attention_mask.shape[0], 36)).to(
            # #     attention_mask.device)
            # loss_mi = 0.
            # from exp_analysis.rsa_utils import pearson_corr
            # p_corr_dec = []
            # p_corr_enc = []
            # for ii in range(12):
            #     h_a = F.normalize(decoder_outputs.ups[ii].mean(1))
            #     h_a_sim = torch.mm(h_a, h_a.t())
            #     up_index = torch.triu_indices(h_a_sim.shape[0], h_a_sim.shape[1])
            #     h_a_sim = h_a_sim[up_index[0], up_index[1]]
            #
            #     h = F.normalize(sequence_output.mean(1))
            #     h_sim = torch.mm(h, h.t())
            #     h_sim = h_sim[up_index[0], up_index[1]]
            #
            #     p_corr_dec.append(pearson_corr(h_a_sim, h_sim).item())
            #
            #     h_a = F.normalize(encoder_outputs.ups[ii].mean(1))
            #     h_a_sim = torch.mm(h_a, h_a.t())
            #     up_index = torch.triu_indices(h_a_sim.shape[0], h_a_sim.shape[1])
            #     h_a_sim = h_a_sim[up_index[0], up_index[1]]
            #
            ...
            #     h_sim = h_sim[up_index[0], up_index[1]]
            #
            #     p_corr_enc.append(pearson_corr(h_a_sim, h_sim).item())

            #     dec_a_loss = loss_func(
            #         decoder_outputs.ups[ii],
            #         sequence_output.mean(1),
            #         torch.ones((sequence_output.shape[0],
            #                     sequence_output.shape[1])
            #                    ).to(sequence_output.device).bool()
            #     )
            #     # dec_a_loss = symmetric_KL_loss(
            #     #     decoder_outputs.ups[ii].view(-1, 768),
            #     #     sequence_output.view(-1, 768)
            #     # )
            #     loss_mi += dec_a_loss
            # adapter_align_loss = self.config.align_w * loss_mi
            if self.config.using_mi_loss:
                # adapter_align_loss = self.config.align_w * (
                #     torch.stack(decoder_outputs.ada_align_loss).mean()
                # )
                adapter_align_loss = self.config.align_w * 0.5 * (
                    torch.stack(encoder_outputs.ada_align_loss).mean() +
                    torch.stack(decoder_outputs.ada_align_loss).mean()
                ) * 0.2

            # ada_r2loss = adapter_red_loss + adapter_align_loss

        balance_loss = None
        if self.training and self.config.apply_phm_moe and self.config.add_gate:
            if len(encoder_outputs.balance_loss) > 0:
                balance_loss = 0.5 * (torch.stack(
                    encoder_outputs.balance_loss).mean() + torch.stack(
                    decoder_outputs.balance_loss).mean())
        
        loss_consistency = None
        if self.config.use_consistency_loss and self.training:
            encoder_outputs = self.encoder(
                input_ids=input_ids,
                attention_mask=attention_mask,
                inputs_embeds=inputs_embeds,
            ...
            logits_additional = self.lm_head(sequence_output)
            if labels is not None:
                loss_consistency = symmetric_KL_loss(
                    lm_logits.view(-1, lm_logits.size(-1)),
                    logits_additional.view(-1, lm_logits.size(-1))
                )
        ...
        return VLSeq2SeqLMOutput(
            ...
            # ada_r2loss=ada_r2loss,
            ada_align_loss=adapter_align_loss,
            ada_red_loss=adapter_red_loss,
    ...
    def _expand_inputs_for_generation(
        ...
        if model_kwargs.get("vis_attention_mask", None) is not None:
            model_kwargs['vis_attention_mask'] = model_kwargs[
                'vis_attention_mask'].index_select(
                0, expanded_return_idx)
...
class VLSeq2SeqLMOutput(ModelOutput):
    ...
    ada_r2loss: Optional[torch.FloatTensor] = None
    ada_red_loss: Optional[torch.FloatTensor] = None
    ada_align_loss: Optional[torch.FloatTensor] = None
