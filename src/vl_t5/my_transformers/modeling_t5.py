...
from vl_t5.adaMix.adapter import Adapter
from vl_t5.adaMix.expert_soup import ExpertSoup
from vl_t5.compacter.adapter_modeling import HyperComplexAdapter, LowRankAdapter

from vl_t5.mixphm.pw_moe import MoEAdaptor
from vl_t5.mixphm.phm_moe import PHMAdapterMoE
from vl_t5.mixphm.adapter_moe import MultiDownAdapterMoE
...
def load_tf_weights_in_t5(model, config, tf_checkpoint_path):
    ...
    for txt_name in names:
        ...
        for m_name in name:
            ...
            elif scope_names[0] == "wi" and len(scope_names) > 1 and scope_names[
                1].isdigit():
...
class T5LayerNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6, config=None):
        ...
        self.bitfit = config.bitfit if config is not None else False
        if self.bitfit:
    ...
    def forward(self, hidden_states):
        ...
        # https://arxiv.org/abs/1910.07467 thus varience is calculated
        # w/o mean and there is no bias.
        # Additionally we want to make sure that the accumulation for
...
class T5DenseReluDense(nn.Module):
    ...
    def forward(self, hidden_states):
        hidden_states = self.wi(hidden_states)  # [bs, seq_len, d_ff]
...
class T5LayerFF(nn.Module):
    def __init__(self, config: T5Config):
        ...
        if config.feed_forward_proj == "relu":
            ...
            raise ValueError(
                ...
                f"Choose between `relu` and `gated-gelu`"
            )
        ...
        if config.apply_moe:
            self.AdaMoE = MoEAdaptor(
                config.adapter_size,
                act='gelu',
                num_expert=config.num_experts,
            )
        
        if config.apply_adapter_moe:
            self.ada_moe = MultiDownAdapterMoE(
                config.hidden_size,
                config.adapter_size,
                num_expert=config.num_experts,
                inference_level=config.inference_level,
        ...
        if config.apply_compacter:
            if config.hypercomplex_adapter:
                self.phm_adapter = HyperComplexAdapter(config)
            elif config.low_rank_adapter:
                self.phm_adapter = LowRankAdapter(config)
        
        if config.apply_phm_moe:
            self.phm_adapter_moe = PHMAdapterMoE(
                config,
    ...
    def forward(self, hidden_states):
        ...
        forwarded_states = self.DenseReluDense(forwarded_states)  # FFN
        ...
        (up, residual) = (None, None)
        if hasattr(self, 'ExpertSoup'):
            if self.config.use_adapter_r2loss:
                forwarded_states, (up, residual) = self.ExpertSoup(
                    forwarded_states, is_r2loss=True
                )
            else:
                forwarded_states = self.ExpertSoup(
                    forwarded_states
                )
        
        if hasattr(self, 'AdaMoE'):
            forwarded_states = self.AdaMoE(forwarded_states)
        
        if hasattr(self, 'ada_moe'):
            forwarded_states = self.ada_moe(forwarded_states)
        
        if hasattr(self, 'phm_adapter'):
            forwarded_states = self.phm_adapter(forwarded_states)
        
        if hasattr(self, 'phm_adapter_moe'):
            if self.config.use_adapter_r2loss:
                forwarded_states, (up, residual) = self.phm_adapter_moe(
                    forwarded_states, is_r2loss=True
                )
            else:
                forwarded_states, balance_loss = self.phm_adapter_moe(
                    forwarded_states)
...
class T5LayerSelfAttention(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        ...
        self.layer_norm = T5LayerNorm(
            config.d_model, eps=config.layer_norm_epsilon, config=config
        )
        self.dropout = nn.Dropout(config.dropout_rate)
        ...
        if config.apply_compacter and config.use_adapter_self_attn:
            if config.hypercomplex_adapter:
                self.self_attn_phm_adapter = HyperComplexAdapter(config)
            elif config.low_rank_adapter:
                self.self_attn_phm_adapter = LowRankAdapter(config)
    ...
    def forward(
        ...
        if hasattr(self, 'self_attn_phm_adapter'):
            y = self.self_attn_phm_adapter(y)
...
class T5LayerCrossAttention(nn.Module):
    def __init__(self, config):
        ...
        if config.apply_expert_soup and config.use_adapter_cross_attn:
            self.ExpertSoup = ExpertSoup(
                config.hidden_size,
                config.adapter_size,
                'gelu',
        ...
        if config.apply_compacter and config.use_adapter_cross_attn:
            if config.hypercomplex_adapter:
                self.cross_attn_phm_adapter = HyperComplexAdapter(config)
            elif config.low_rank_adapter:
                self.cross_attn_phm_adapter = LowRankAdapter(config)
    ...
    def forward(
        ...
        if hasattr(self, 'cross_attn_phm_adapter'):
            y = self.cross_attn_phm_adapter(y)
    ...
    def forward(
        ...
        if past_key_value is not None:
            if not self.is_decoder:
                logger.warning("`past_key_values` is passed to the encoder. "
                               "Please make sure this is intended.")
            expected_num_past_key_values = 2 \
                if encoder_hidden_states is None else 4
        ...
        balance_loss = None
        if self.config.use_adapter_r2loss:
            hidden_states, (up, residual) = self.layer[-1](hidden_states)
        else:
            hidden_states, balance_loss = self.layer[-1](hidden_states)
...
class T5PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, T5LayerNorm):
            ...
            module.wi.weight.data.normal_(mean=0.0, std=factor * (
                    (self.config.d_model) ** -0.5))
            if hasattr(module.wi, "bias") and module.wi.bias is not None:
                module.wi.bias.data.zero_()
            module.wo.weight.data.normal_(mean=0.0, std=factor * (
                    (self.config.d_ff) ** -0.5))
            if hasattr(module.wo, "bias") and module.wo.bias is not None:
                module.wo.bias.data.zero_()
        elif isinstance(module, T5DenseGatedGeluDense):
            module.wi_0.weight.data.normal_(mean=0.0, std=factor * (
                    (self.config.d_model) ** -0.5))
            if hasattr(module.wi_0, "bias") and module.wi_0.bias is not None:
                module.wi_0.bias.data.zero_()
            module.wi_1.weight.data.normal_(mean=0.0, std=factor * (
                    (self.config.d_model) ** -0.5))
            ...
            module.q.weight.data.normal_(mean=0.0, std=factor * (
                    (d_model * key_value_proj_dim) ** -0.5))
            module.k.weight.data.normal_(mean=0.0, std=factor * (d_model ** -0.5))
            module.v.weight.data.normal_(mean=0.0, std=factor * (d_model ** -0.5))
            module.o.weight.data.normal_(mean=0.0, std=factor * (
    ...
    def _shift_right(self, input_ids):
        ...
        assert torch.all(
            shifted_input_ids >= 0).item(), "Verify that `shifted_input_ids` has only positive values"
...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None):
        ...
        # TODO
        if self.config.use_adapter_r2loss:
            from vl_t5.losses.r2_loss import R2Loss
            self.r2_loss_fun = R2Loss()
    ...
    def parallelize(self, device_map=None):
        ...
        self.first_device = "cpu" if "cpu" in self.device_map.keys() else "cuda:" + str(
            min(self.device_map.keys()))
    ...
    def forward(
        ...
        if input_ids is not None and inputs_embeds is not None:
            ...
            raise ValueError(
                f"You cannot specify both {err_msg_prefix}input_ids "
            ...
            err_msg_prefix = "decoder_" if self.is_decoder else ""
            raise ValueError(
                f"You have to specify either {err_msg_prefix}input_ids "
                f"or {err_msg_prefix}inputs_embeds")
        ...
        if use_cache is True:
            assert self.is_decoder, f"`use_cache` can only be set to `True` " \
                                    f"if {self} is used as a decoder"
        ...
        if self.is_decoder and encoder_attention_mask is None \
            ...
            encoder_attention_mask = torch.ones(
                batch_size, encoder_seq_length, device=inputs_embeds.device,
                dtype=torch.long
            )
        ...
        all_cross_attentions = () if (
                output_attentions and self.is_decoder) else None
        position_bias = None
        encoder_decoder_position_bias = None
        ...
        for i, (layer_module, past_key_value) in enumerate(
            ...
            if self.model_parallel:
                ...
                # Ensure that attention_mask is always
                # on the same device as hidden_states
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if self.config.use_adapter_r2loss:
                    layer_outputs, (up, residual) = layer_module(
                        hidden_states,
                    ...
                    layer_outputs, balance_loss = layer_module(
                        hidden_states,
                        attention_mask=extended_attention_mask,
                        position_bias=position_bias,
                        encoder_hidden_states=encoder_hidden_states,
        ...
        # TODO
        ada_red_loss, ada_align_loss = [], []
        if self.training and self.config.use_adapter_r2loss:
            for n_l in range(len(ups)):
                red_loss = self.r2_loss_fun(
                    ups[n_l],
                    residuals[n_l],
                    attention_mask.int(),
                )
                ada_red_loss.append(red_loss)

                if self.config.using_mi_loss:
                    loss_mi = self.r2_loss_fun.mi_loss(
                        ups[n_l],
                        hidden_states.mean(1),
                        attention_mask.int(),
                        measure=self.config.mi_measure
                    )
                    ada_align_loss.append(loss_mi)
        ...
        return BaseModelOutputWithPastAndCrossAttentions(
            ...
            ada_red_loss=ada_red_loss,
            ada_align_loss=ada_align_loss,
            balance_loss=balance_losses,
            # ups=[ups[i] for i in range(12)] if self.training else ups
        )
...
class BaseModelOutputWithPastAndCrossAttentions(ModelOutput):
    ...
    ada_red_loss: Optional[List[torch.FloatTensor]] = None
    ada_align_loss: Optional[List[torch.FloatTensor]] = None
    balance_loss: Optional[List[torch.FloatTensor]] = None
    ups: Optional[List[torch.FloatTensor]] = None
