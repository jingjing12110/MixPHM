...
class SparseDispatcher(object):
    ...
    def combine(self, expert_out, multiply_by_gates=True):
        ...
        # add eps to all zero values in order
        # to avoid nans when going back to log space
...
class MoE(nn.Module):
    ...
    def __init__(self, input_size, output_size, num_experts, hidden_size,
        ...
        self.experts = nn.ModuleList(
            [MLP(self.input_size, self.output_size, self.hidden_size) for i in
    ...
    def noisy_top_k_gating(self, x, train, noise_epsilon=1e-2):
        ...
        top_logits, top_indices = logits.topk(min(self.k + 1, self.num_experts),
                                              dim=1)
    ...
    def forward(self, x, train=True, loss_coef=1e-2):
        ...
        expert_outputs = [self.experts[i](expert_inputs[i]) for i in
                          range(self.num_experts)]
