...
class MoELayer(nn.Module):
    def __init__(self, hidden_size, num_experts, expert, route_method, vocab_size,
                 hash_list):
    ...
    def _random_hash_list(self, vocab_size):
        hash_list = torch.randint(low=0, high=self.num_experts,
                                  size=(vocab_size,))
        return hash_list
    ...
    def _forward_gate_token(self, x):
        ...
        x = x.split(num_tokens.tolist(),
                    dim=0)  # a list of length self.num_experts
    ...
    def _forward_gate_sentence(self, x, attention_mask):
        ...
        x = x[order]  # reorder according to expert number
        x = x.split(num_sentences.tolist(),
                    dim=0)  # a list of length self.num_experts
    ...
    def forward(self, x, input_ids=None, attention_mask=None):
        if self.route_method == "gate-token":
            ...
            if x.size(0) == 1:
                x, balance_loss, gate_load = self._forward_sentence_single_expert(
                    x, attention_mask)
            else:
                x, balance_loss, gate_load = self._forward_gate_sentence(
                    x, attention_mask)
